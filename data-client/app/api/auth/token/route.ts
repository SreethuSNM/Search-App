...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("Webflow API Response:", response.status, await response.text());
     ...
     // Check Webflow response status
     if (!response.ok) {
      const errorResponse = await response.json();
      console.error("Error from Webflow:", errorResponse);
      return NextResponse.json({ error: "Failed to verify user with Webflow" }, { status: 401 });
    }
